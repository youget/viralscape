...
import { Search, X, Share2, Heart, Play, Loader2 } from 'lucide-react'
...
function getCacheKey(type, query) {
  return `${CACHE_KEY}-${type}-${query || 'trending'}`
}
...
export default function VideosPage() {
  ...
  async function loadVideos(type, q) {
  ...
  return (
    <div className="px-4 py-6 max-w-2xl mx-auto">
      ...
      <div className="flex gap-2 mb-4">
        <button
          className="flex-1 py-2.5 rounded-xl text-xs font-bold vs-btn"
        >
          YouTube
        </button>
        <button
          onClick={() => setPopup('tiktok')}
          className="flex-1 py-2.5 rounded-xl text-xs font-bold vs-btn-outline"
        >
          TikTok
        </button>
        <button
      ...
      <form onSubmit={handleSearch} className="relative mb-4">
        <input
          ...
          className="w-full py-3 px-4 pr-12 rounded-xl vs-card border vs-border text-sm vs-text outline-none focus:border-[var(--vs-accent)] transition-colors"
          style={{ backgroundColor: 'var(--vs-card)' }}
        />
        <button
          type="submit"
          className="absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 flex items-center justify-center rounded-lg vs-hover transition-colors"
          ...
          {videos.map((v, i) => (
            ...
            >
              <div className="relative aspect-video">
                ...
                {isFav(v.id) && (
                  <span className="absolute top-1 right-1 text-[10px] px-1.5 py-0.5 rounded bg-red-500/90 text-white">
                    ♥
                  </span>
                )}
              </div>
        ...
        >
          ...
          <div className="p-4 bg-black/95">
            ...
            <p className="text-xs text-gray-400 mb-4">
              {activeVideo.channel} • {formatViews(activeVideo.views)} views
            </p>
            ...
            <div className="flex items-center justify-center gap-4">
              ...
              <button
                onClick={() => handleFav(activeVideo)}
                className="flex flex-col items-center gap-1 px-4 py-2 rounded-xl transition-colors"
              >
                <Heart
                  size={22}
                  fill={isFav(activeVideo.id) ? 'var(--vs-accent)' : 'none'}
                  style={{ color: 'var(--vs-accent)' }}
                />
                <span className="text-[10px] font-semibold text-gray-400">
                  {isFav(activeVideo.id) ? 'Saved' : 'Save'}
                </span>
              </button>
            </div>
          </div>
